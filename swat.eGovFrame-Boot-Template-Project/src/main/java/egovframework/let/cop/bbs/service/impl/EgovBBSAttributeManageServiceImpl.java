...
public class EgovBBSAttributeManageServiceImpl extends EgovAbstractServiceImpl
	...
	private final BBSAttributeManageReadOnlyDAO bbsAttributeManageReadOnlyDAO;
...
//		CustomerContextHolder.setCustomerType(CustomerType.READ);
//
//		try {
		...
		DatabaseMetaData databaseMetaDataReadOnly = bbsAttributeManageReadOnlyDAO.getSqlSession().getConnection()
				.getMetaData();
		...
		if (log.isDebugEnabled()) {
			...
			log.debug("");
			log.debug("1. getURL={}", databaseMetaDataReadOnly.getURL());
			log.debug("1. getUserName={}", databaseMetaDataReadOnly.getUserName());
		}

		BoardMasterVO result = attrbMngDAO.selectBBSMasterInf(searchVO);
		bbsAttributeManageReadOnlyDAO.selectBBSMasterInf(searchVO);
		...
		if (log.isDebugEnabled()) {
			...
			log.debug("");
			log.debug("2. getURL={}", databaseMetaDataReadOnly.getURL());
			log.debug("2. getUserName={}", databaseMetaDataReadOnly.getUserName());
		}
