...
@Configuration
@Slf4j
public class EgovConfigAppDatasourceReadOnly {
	...
	@PostConstruct
	void init() {
		dbType = env.getProperty("Globals.DbTypeReadOnly");
		// Exception 처리 필요
	 ...
	 */
	@Bean(name = { "dataSourceReadOnly", "egov.dataSourceReadOnly", "egovDataSourceReadOnly" })
	...
	DataSource routingDataSource(
		...
		AbstractRoutingDataSource routingDataSource = new AbstractRoutingDataSource() {
			...
			protected Object determineCurrentLookupKey() {
				...
				CustomerType customerType = CustomerContextHolder.getCustomerType();
				...
				if (customerType == null) {
					...
					if (readOnly) {
						return CustomerType.READ;
					} else {
						return CustomerType.WRITE;
					}
				} else {
					return customerType;
				}
		...
		targetDataSources.put(CustomerType.WRITE, dataSource);
		targetDataSources.put(CustomerType.READ, dataSourceReadOnly);
	...
	@Bean
	DataSource lazyConnectionDataSource(DataSource routingDataSource) {
